...
| vault.server.affinity | string | `nil` |  |
| vault.server.dataStore.enabled | bool | `false` |  |
| vault.server.dev | object | `{"enabled":true}` | This is just for quick install. For a production, NEVER EVER use the vault server in dev mode. |
| vault.server.resources.requests.cpu | string | `"25m"` |  |
| vault.server.resources.requests.memory | string | `"64Mi"` |  |
| vault.server.tolerations | string | `nil` |  |
